...
export default function ChatWindow({ coachId, threadId }: ChatWindowProps) {
  ...
  useEffect(() => {
    if (!threadId) return;
    fetch(`/api/coaches/${coachId}/chats/threads/${threadId}`)
      .then(res => res.json())
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/coaches/${coachId}/chats/threads/${threadId}`, {
        method: 'POST',
